...
_.each(internals, function(internalType, name) {
  utils.addChainableMethod(ovt, name, function() {
    return new internals[name]();
  }, { type: 'internal' });
});

ovt.addMethod = function addMethod(type, name, method, options) {
  assert(~validTypes.indexOf(type), `${type} is not a valid schema type.`);

  utils.addChainableMethod(internals[type].prototype, name, method, options);
};
