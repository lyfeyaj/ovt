...
class ObjectType extends AnyType {
  constructor() {
    ...
    this._addSchemas.apply(this, arguments);
  }
  ...
  _addSchema(name, schema) {
    ...
    // check array type
      ...
      let arraySchema = utils.applyType(ArrayType, schema);
      this._inner.children[name] = arraySchema;
    }

    // check plain object type
    else if (utils.isObject(schema)) {
      let objectSchema = utils.applyType(ObjectType);
      objectSchema = objectSchema._addSchemas(schema);
