...
proto._validateInner = function(val, state, options) {
  ...
  let exclusions = self._inner.exclusions.slice();
  let requireds = self._inner.requireds.slice();
  let inclusions = self._inner.inclusions.slice().concat(requireds);
  let ordereds = self._inner.ordereds.slice();
  ...
  value = _.map(value, function(item, ii) {
    ...
    let currentPath = utils.buildPath(state, { parentPath, key: ii });

    // Validate exclusions
    _.each(exclusions, function(exclusion) {
      let res = exclusion._validate(item, utils.buildState(state, { hasErrors: errors.any() || state.hasErrors }), options);
    ...
    if (ordered) {
      let res = ordered._validate(item, utils.buildState(state, { hasErrors: errors.any() || state.hasErrors }), options);
      item = res.value;
    ...
    _.some(requireds.slice(), function(required, ri) {
      let res = required._validate(item, utils.buildState(state, { hasErrors: errors.any() || state.hasErrors }), options);
      if (!res.errors) {
        item = res.value;
    ...
    _.some(inclusions, function(inclusion) {
      ...
      let res = inclusion._validate(item, utils.buildState(state, { hasErrors: errors.any() || state.hasErrors }), options);
