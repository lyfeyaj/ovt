...
chainable('when', {
  ...
  parseArgs: function(ref, condition) {
    ...
    utils.assert((condition.then && condition.then.isOvt), 'condition.then must be a valid ovt schema');
    utils.assert((condition.otherwise && condition.otherwise.isOvt), 'condition.otherwise must be a valid ovt schema');
  ...
  method: function(val, ref, condition) {
    console.log(val, ref, condition);
    condition = condition || {};
    let res = {};
    let is = condition.is;
    ...
    if (is === ref) {
      if (then && then.isOvt) res = then.validate(val, this.options);
    } else {
