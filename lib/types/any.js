...
AnyType.prototype.clone = function() {
  ...
  obj.isOvt = true;
  obj._methods = bue.extend({}, this._methods);
...
AnyType.prototype._validate = function _validate(val, options) {
  options = options || {};
  ...
  bue.each(methods, function(method) {
    var args = (method.args || []).slice();
    args.unshift(val);

    var result = method.fn.apply(method, args);
