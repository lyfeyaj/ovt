...
class Schema {
  ...
  isEmpty(val) {
    let emptySchema = this._emptySchema;

    let condition = utils.isUndefined(val);

    if (condition) return condition;

    if (emptySchema && emptySchema.isOvt) {
      let res = emptySchema.validate(val, { abortEarly: true });
      condition = condition || !res.errors;
    } else if (!utils.isUndefined(emptySchema)) {
      condition = condition || isEqual(val, emptySchema);
    }
  ...
  validate(value, options, state) {
    ...
    return Processor.execute(value, this, state, options);
  }
...
chainable('options', {
  chainingBehaviour: function addOptions(opts) {
    opts = opts || {};
