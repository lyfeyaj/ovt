  ...
  execute() {
    ...
    value = (value || []).map(function(item, vi) {
      ...
      for (let ei = 0; ei < exclusions.length; ei++) {
        ...
        let res = Calculator.execute(item, exclusion, self.stateBuilder(index), self.options);
      ...
      if (ordered) {
        let res = Calculator.execute(item, ordered, self.stateBuilder(index), self.options);
        item = res.value;
      ...
      if (orderedExclusion) {
        let res = Calculator.execute(item, orderedExclusion, self.stateBuilder(index), self.options);
      ...
      for (let ri = 0; ri < requireds.length; ri++) {
        ...
        let res = Calculator.execute(item, required, self.stateBuilder(index), self.options);
      ...
      for (let ii = 0; ii < inclusions.length; ii++) {
        ...
        let res = Calculator.execute(item, inclusion, self.stateBuilder(index), self.options);
    ...
    if (ordereds.length > value.length) {
      for (let oi = value.length; oi < ordereds.length; oi++) {
        ...
        let index = oi + '';
        if (ordered) {
          let res = Calculator.execute(undefined, ordered, this.stateBuilder(index), self.options);
