...
class BaseCalculator {
  ...
  createError(key, e, msg) {
    this.errors.add(msg ? msg : this.buildPath(this.path, key), e);
  }
  ...
  execMethods() {
    ...
    for (let name in this.methods) {
      ...
      if (name === '__inners_flag__' && method) {
        ...
        if (this.canSkipMethodBy(method.type)) continue;
        ...
        let _res = utils.invokeMethod(method, value, state);
        ...
        if (method.type === 'validator') {
          if (utils.isError(_res)) {
            ...
            let errorMessage = utils.m(method, this.options.locale) ||
                               utils.t(`${this._type}.${name}`, args) ||
                               _res;
            this.createError(name, errorMessage, this._description);
          }
        } else {
          value = _res;
        }
